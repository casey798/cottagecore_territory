...
import { WORD_GROUPS_TIME_LIMIT } from '@/constants/config';
import { generateClientCompletionHash } from '@/utils/hmac';
import type { MinigamePlayProps, MinigameResult } from '@/types/minigame';
import { generatePuzzle, checkGroup, MAX_MISTAKES } from './WordGroupsLogic';
import type { WordGroupsPuzzle } from './WordGroupsLogic';
...
export default function WordGroupsGame(props: MinigamePlayProps) {
  const { sessionId, timeLimit, onComplete, practiceMode } = props;
  const effectiveTimeLimit = timeLimit > 0 ? timeLimit : WORD_GROUPS_TIME_LIMIT;

  // Generate puzzle client-side on mount
  const puzzleRef = useRef<WordGroupsPuzzle | null>(null);
  ...
  // Bug fix: mirror mistakes and solvedGroups in refs to avoid stale closures
      ...
      >
        <Pressable style={styles.modalBackdrop} onPress={closeHowToPlay}>
          <Pressable style={styles.modalPanel} onPress={() => {}}>
            ...
            <Text style={styles.modalTitle}>How to Play {'\u2014'} Word Groups</Text>
            <Text style={styles.modalRule}>
              {'\u2022'} 16 words are shown in a grid. Find 4 groups of 4 words that share something in common.
            </Text>
            <Text style={styles.modalRule}>
              {'\u2022'} Tap a word to select it. Tap it again to deselect it. You can select up to 4 words at once.
            </Text>
            <Text style={styles.modalRule}>
              {'\u2022'} When you have 4 words selected, they are submitted automatically.
            </Text>
            <Text style={styles.modalRule}>
              {'\u2022'} A correct group is revealed immediately with its category name.
            </Text>
            <Text style={styles.modalRule}>
              {'\u2022'} An incorrect guess costs one mistake. You have 8 mistakes before the game ends.
            </Text>
            <Text style={styles.modalRule}>
              {'\u2022'} Find all 4 groups to win.
            </Text>
            <Text style={styles.modalTip}>
              Tip: If 3 of your selected words look right but the 4th is wrong, try swapping out the one that could belong to a different group.
            </Text>
          </Pressable>
        </Pressable>
      </Modal>
